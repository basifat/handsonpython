# The project is about a simple database table application that allows a user to perform crud operations
# Table abtsraction allows other table to be created
# CRUD -> Create (add), Retrieve (get), Update, Delete
# The objectives:
# A user can add a new record to the table
# A User can update an existing record on the table
# A User can delete an existing record on the table
# A user can deduplicate records on the table 
# A table can query/search for a specific record
# A table can be populated or loaded with records from a CSV

#solution:
#1. created a BaseTable class that will allow subclasses to inherit properties of the base table class.
#2.@abstractmethod decorator enforces all methods in the base class to be implemented by subclasses
